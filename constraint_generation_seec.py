...
class ConstraintGeneratorSeec:
    def __init__(self, config):
        self.config = config
        self.graph_config = {
            "configurable": GraphConfig(
                high_level_actor=config["high_level_actor"],
                high_level_critic=config["high_level_critic"],
                low_level_actor=config["low_level_actor"],
                cypher_query=config["cypher_query"],
                low_level_critic=config["low_level_critic"],
                codegen_actor=config["codegen_actor"],
                codegen_critic=config["codegen_critic"],
    ...
    def generate(self, img, instruction, metadata):
        ...
        final_state = {k: v for k, v in self.graph.get_state(self.graph_config).values.items() if k != "messages"}
        output = final_state.get("code")
        # save raw output
